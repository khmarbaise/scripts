#!/bin/bash
LOAD_RRD=/usr/share/rrdtool/load.rrd
VHOST=/usr/local/vhosts/monitor
RRDTOOL=/usr/bin/rrdtool

case $1 in (create)
		$RRDTOOL create $LOAD_RRD -s 60 \
	...
	(graph)
		...
		DEF:user=$LOAD_RRD:cpuuser:AVERAGE \
		DEF:nice=$LOAD_RRD:cpunice:AVERAGE \
		DEF:sys=$LOAD_RRD:cpusystem:AVERAGE \
		...
		-Y -r -u 100 -l 0 -L 5 -v "CPU usage" -w 700 -h 300 -t "CPU stats - `/bin/date`" \
		-c ARROW\#000000 -x MINUTE:30:MINUTE:30:HOUR:1:0:%H \
		DEF:user=$LOAD_RRD:cpuuser:AVERAGE \
		DEF:nice=$LOAD_RRD:cpunice:AVERAGE \
		DEF:sys=$LOAD_RRD:cpusystem:AVERAGE \
